...
## Check before saving
```{r}
summary(env.data.ann.summ)
```


## Reconcile names
```{r}
names(env.data.ann.summ)[2] <- "habitat"
```
...
```{r}
write.csv(env.data.ann.summ,
          file = "data/data_out/env_data_annual_and_seasonal_summaries_w_lag.csv",
          row.names = F)
```
